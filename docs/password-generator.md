...
- PIN digits are centered in a single line with extra letter spacing and an adaptive 18–28 fp monospace font. Hidden digits use the same centered layout. Spacing is visual only: copying uses the original continuous numeric string, including leading zeros.
